      ...
      .flatMap { groupedFlux ->
        groupedFlux.collectList().map { results ->
          PaymentEvent(
            ...
            isPaymentDone = (results.first()["is_payment_done"] as Boolean),
            paymentOrders = results.map { result ->
              PaymentOrder(
                ...
                isLedgerUpdated = (result["ledger_updated"]) as Boolean,
                isWalletUpdated = (result["wallet_updated"]) as Boolean
              )
            }
          )
        }
